...
def draw_circle(coordinates, color):
    for coordinate in coordinates:
        cv2.circle(image, (int(coordinate[0]), int(coordinate[1])), coordinate[2], color, 2)
        cv2.circle(image, (int(coordinate[0]), int(coordinate[1])), 2, color, 3)
...
def chess(image):
    global white_coordinates, black_coordinates

    circle_image = image.copy()
    # 检测圆
    circles = cv2.HoughCircles(cv2.cvtColor(circle_image, cv2.COLOR_BGR2GRAY), cv2.HOUGH_GRADIENT, dp=1.2, minDist=20,
...
def chess_board(image):
    global white_coordinates, black_coordinates, white_chess_coordinates, black_chess_coordinates
    # 转换为灰度图像
    ...
    if filtered_contours:
        ...
        for coordinate in white_coordinates:
            ...
            wx = wx - x
            wy = wy - y
            if 0 <= int(wx) <= cropped_image.shape[0] and 0 <= int(wy) <= cropped_image.shape[1]:
    ...
    # 画不在棋盘内的圆
    draw_circle(white_coordinates, (0, 0, 255))
    draw_circle(black_coordinates, (0, 0, 255))
    # 画在棋盘内的圆
    draw_circle(white_chess_coordinates, (0, 255, 0))
    draw_circle(black_chess_coordinates, (0, 255, 0))
