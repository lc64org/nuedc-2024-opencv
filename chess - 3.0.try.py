...
def the_chess_real_sit(a, b):  # 设机械臂距离屏幕顶端为200mm,距离屏幕两侧都为10mm
    x = 100-0.896*a
    y = 200-0.896*b
    return x, y


def the_square_real_sit(c, d):  # 设机械臂距离同上
    x = 100-0.896*c
    y = 200-0.896*d
    return x, y
...
def summarize_chess_info(grid_centers, white_positions, black_positions):
    summary = []
    for i, center in enumerate(grid_centers):
        ...
        if (row, col) in white_positions:
            ...
            chess_info["棋子坐标"] = white_rot_coordinates[index]
            chess_info["是否在棋盘上"] = "是"

        elif (row, col) in black_positions:
            chess_info["棋子颜色"] = "黑色"
            index = black_positions.index((row, col))
            chess_info["棋子坐标"] = black_rot_coordinates[index]
            chess_info["是否在棋盘上"] = "是"
...
def chess_board(image):
    ...
    if filtered_contours:
        ...
        # 调用总结函数并打印
        summary = summarize_chess_info(grid_centers, white_positions, black_positions)
        for info in summary:
            print(f"格子中心坐标: {info['格子中心坐标']}, 棋子颜色: {info['棋子颜色']}, 棋子坐标: {info['棋子坐标']}, 是否在棋盘上: {info['是否在棋盘上']}")
...
if __name__ == "__main__":
    ...
    cap.set(cv2.CAP_PROP_EXPOSURE, -5.95)
    if not cap.isOpened():
