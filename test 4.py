...
def detect_chess_pieces(hsv_img, grid_coordinates, image, offset_x, offset_y):
    """ 检测棋子颜色并判断出哪个格子中有棋子 """
    ...
    for i, (top_left, bottom_right) in enumerate(grid_coordinates):
        ...
        # 计算相对坐标（相对于原始图像）
        relative_x = center_x_grid - offset_x
        relative_y = center_y_grid - offset_y

        # 在图像上标注格子的编号和相对坐标
        text = f"{i+1}: ({relative_x}, {relative_y})"
        cv2.putText(image, text, (center_x_grid - 30, center_y_grid + 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 0), 2)

        # 打印格子编号和相对坐标
        print(f"格子 {i+1}: 相对坐标 ({relative_x}, {relative_y})")
...
def process_frame(frame):
    ...
    if len(contours) > 0:
        ...
        if len(approx) == 4:
            ...
            # 检测棋子
            detect_chess_pieces(hsv_warped, grid_coordinates, warped, 45, 45)
