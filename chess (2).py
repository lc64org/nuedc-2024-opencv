...
def find_chess_out(image, circles):
    # 将图像转换为灰度图像
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)

    # 应用阈值处理以便更容易找到轮廓
    _, thresh = cv2.threshold(gray, 70, 120, cv2.THRESH_BINARY)
    ...
    if contours:
        ...
        # 5. 绘制轮廓
        cv2.polylines(image, [max_contour], isClosed=True, color=(0, 255, 0), thickness=2)

        # 计算矩形的边界矩形
        x, y, w, h = cv2.boundingRect(max_contour)

        # 确保 circles 是一个圆的信息
        if circles is not None and isinstance(circles, (list, tuple)) and len(circles) == 3:
            # circles 应该是 (cx, cy, r) 的格式
            cx, cy, r = circles
            ...
            if (x <= cx <= x + w) and (y <= cy <= y + h):
                # 读取圆心的颜色以确定棋子的颜色
                piece_color = image[cy, cx]
                piece_hsv = cv2.cvtColor(np.uint8([[piece_color]]), cv2.COLOR_BGR2HSV)[0][0]

                # 判断棋子的颜色
                if cv2.inRange(np.array([piece_hsv]), black_D, black_U):
                    print(f"Black piece at ({cx}, {cy})")
                    cv2.circle(image, (cx, cy), r, (0, 0, 0), 3)  # Draw black border for black piece
                elif cv2.inRange(np.array([piece_hsv]), white_D, white_U):
                    print(f"White piece at ({cx}, {cy})")
                    cv2.circle(image, (cx, cy), r, (255, 255, 255), 3)  # Draw white border for white piece
                else:
                    print(f"Unknown piece color at ({cx}, {cy})")

                return True

    return False  # 如果没有找到满足条件的圆或矩形，则返回 False


def detect_chess_find(frame1, frame2):
    """
    检测棋子并返回其颜色和坐标。

    参数:
        frame (numpy.ndarray): 输入的图像帧，通常是一个BGR图像。

    返回:
        list: 棋子的颜色和坐标列表，每个棋子用一个字典表示，包含 'color' 和 'coordinate' 键。
    """

    # 检测圆形
    circles = cv2.HoughCircles(frame2, cv2.HOUGH_GRADIENT, dp=1.0, minDist=25,
    ...
    if circles is not None:
        ...
        for (x, y, r) in circles:
            ...
            cv2.circle(frame1, (x, y), 2, (0, 0, 255), 3)
            ...
            # 检查区域的像素颜色
            region = frame1[y_min:y_max, x_min:x_max]
            mean_color = np.mean(cv2.cvtColor(region, cv2.COLOR_BGR2GRAY))

            # 判断黑白色
            color = "white" if mean_color > 127 else "black"

            # 保存颜色和坐标
            circle_info.append({'color': color, 'coordinate': (x, y)})

            # 显示结果（可选）
            cv2.putText(frame1, f"{color}", (x - 10, y - r - 10),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 0), 2)
            print(color, int(x), int(y))
            chess_color_sit(color, x, y)


def chess_color_sit(color, x, y):
    chess_color = color
    chess_x = x
    chess_y = y
    return chess_color,chess_x,chess_y
...
def main():
    ...
    while True:
        ...
        detect_chess_find(circle_image, gray)
        # 应用边缘检测
